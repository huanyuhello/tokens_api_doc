...
## 请求参数
Header 参数生成代码X-Forwarded-Hoststring 可选示例:示例值:localhost:5173Acceptstring 必需示例:application/jsonAuthorizationstring 必需示例:sk-Content-Typestring 必需示例:application/jsonBody 参数application/json必填生成代码要使用的模型的 ID。有关哪些模型可与聊天 API 一起使用的详细信息,请参阅模型端点兼容性表。modelstring 必需messagesarray [object] 必需至今为止对话所包含的消息列表。Python 代码示例。

rolestring 必需contentstring 必需temperatureinteger 可选integer
可选
使用什么采样温度，介于 0 和 2 之间。较高的值（如 0.8）将使输出更加随机，而较低的值（如 0.2）将使输出更加集中和确定。 我们通常建议改变这个或top_p但不是两者。

top_pinteger 可选integer
可选
...
response_formatobject 可选object
可选
