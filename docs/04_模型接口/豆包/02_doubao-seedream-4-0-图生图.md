...
| `image` | string | 必需 | 输入的图片信息，支持 URL 或 Base64 编码。doubao-seedream-4.0 支持单图或多图输入。 图片URL：请确保图片URL可被访问。 Base64编码：请遵循此格式data:image/<图片格式>;base64,<Base64编码>。注意 <图片格式> 需小写，如 data:image/png;base64,<base64_image>。 |
