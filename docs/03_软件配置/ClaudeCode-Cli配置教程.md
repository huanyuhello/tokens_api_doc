...
!!! warning
    一个```Tokens claude code``` 的 [密钥/令牌（key）](https://tokens.smartfashionai.cn/)
...
=== "windows 平台"
    ...
    **方式二：使用图形用户界面 (GUI) (推荐给所有Windows用户)**
    ...
    添加第二个变量再次点击“新建(N)...”。
    ```
    变量名(N): ANTHROPIC_BASE_URL
    变量值(V): https://tokens.smartfashionai.cn/
    点击: “确定”。
    ```
...
=== "mac 平台"
    ...
    export ANTHROPIC_BASE_URL="https://tokens.smartfashionai.cn/"
    ```
