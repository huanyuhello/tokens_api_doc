...
messagesobject 必需temperaturestring 可选使用什么采样温度，介于 0 和 2 之间。较高的值（如 0.8）将使输出更加随机，而较低的值（如 0.2）将使输出更加集中和确定。 我们通常建议改变这个或top_p但不是两者。
...
示例
## 返回响应
🟢200成功application/json生成代码Body生成代码idstring ID 编号必需objectstring 必需createdstring 必需choicesarray[string]必需indexstring 必需messageobject 必需finish_reasonstring 必需usagestring 必需prompt_tokensstring 必需completion_tokensstring 必需total_tokensstring 必需请求示例请求示例ShellJavaScriptJavacURLcURL-WindowsHttpiewgetPowerShell
```
